...
def gendata(data_path, label_path, out_path, part='test', config='27'):
    ...
    for line in label_file.readlines():
        ...
        # print(labels[-1])
    '''
    data.append(os.path.join(data_path, '0.mp4.npy'))
...
def run(input_path = None, output_path = None):
    gendata(input_path, "PLACEHOLDER_LABELS", output_path, "test", "27")


def original_main():
    parser = argparse.ArgumentParser(description='Sign Data Converter.')
    parser.add_argument('--data_path', default='D:/npy/Test') #'train_npy/npy', 'va_npy/npy'
    parser.add_argument('--label_path', default='D:/csvs/test_labels.csv') # 'train_labels.csv', 'val_gt.csv', 'test_labels.csv'
    parser.add_argument('--out_folder', default='D:/sign_gen')
