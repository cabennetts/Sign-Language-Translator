...
# @snoop
def crop(image, center, radius, size=1920):
    # scale = 1.3
    scale = 1.3
    print("Center", center)
    print("Radius", radius)
    radius_crop = (radius * scale).astype(np.int32)
    print("Radius Crop", radius_crop)
    ...
    rect = (max(0,(center_crop-radius_crop)[0]), 
            ...
            min(1920,(center_crop+radius_crop)[0]), 
            min(1920,(center_crop+radius_crop)[1]))
                #  min(480,(center_crop+radius_crop)[0]), min(640,(center_crop+radius_crop)[1]))
